...
# MVT Alert Manager Lambda Deployment Script
# Deploys the alert threshold checking Lambda that monitors KPI updates and sends notifications

# Configuration
HANDLER_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FUNCTION_NAME="mvt-alert-manager"
RUNTIME="python3.12"
...
echo -e "${YELLOW}=== MVT Alert Manager Lambda Deployment ===${NC}"
...
if [ "$FUNCTION_EXISTS" != "false" ]; then
    ...
    aws lambda create-function \
        ...
        --description "MVT alert threshold checking and notification handler" \
        --environment "Variables={
            DISTRESS_THRESHOLD=70,
            TRIGGER_THRESHOLD=60,
            CONTAGION_THRESHOLD=80,
            VIX_THRESHOLD=35
        }" \
        > /dev/null
...
# Update environment variables with default thresholds
...
echo -e "${YELLOW}Looking up DynamoDB tables...${NC}"
...
echo "  DISTRESS_THRESHOLD: 70"
echo "  TRIGGER_THRESHOLD: 60"
echo "  CONTAGION_THRESHOLD: 80"
echo "  VIX_THRESHOLD: 35"
echo ""

echo "Environment Variables Required (optional):"
echo "  TELEGRAM_BOT_TOKEN: <Telegram bot token> (must be set manually)"
echo "  TELEGRAM_CHAT_ID: <Telegram chat ID> (must be set manually)"
echo "  GCP_PROJECT_ID: <GCP project ID> (optional, for Pub/Sub)"
echo "  GCP_PUBSUB_TOPIC: mvt-alerts (optional)"
echo "  GCP_SA_KEY_JSON: <service account key JSON> (optional, for Pub/Sub)"
echo ""

echo "To set Telegram credentials:"
cat << 'EOF'
  aws lambda update-function-configuration --function-name mvt-alert-manager \
    --environment "Variables={DISTRESS_THRESHOLD=70,TRIGGER_THRESHOLD=60,CONTAGION_THRESHOLD=80,VIX_THRESHOLD=35,TELEGRAM_BOT_TOKEN=<bot_token>,TELEGRAM_CHAT_ID=<chat_id>}"
EOF
echo ""
